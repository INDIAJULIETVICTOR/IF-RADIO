...
# -----------------------------------------------------------------------------
def set_band(band, frequency, step):

    set_frequency(frequency)

    # Spegni tutti i pulsanti e accendi solo quello selezionato
    # for btn in radio_panel.band_buttons:  # Itera sui pulsanti definiti in band_buttons
        # key = btn["text"]
        # radio_panel.cambia_stato(radio_panel.pulsanti[key], 0)  # Spegni gli altri
        # if key == band:
            # radio_panel.cambia_stato(radio_panel.pulsanti[key], 1)  # Accendi il pulsante
            # time.sleep(0.5)
           # radio_panel.cambia_stato(radio_panel.pulsanti[key], 0)  # Spegni il pulsante
...
class Toplevel1:
    ...
    def __init__(self, top=None):
        ...
        # Pulsanti bandplan
        self.band_buttons = [
            {"text": "21", "command": lambda: set_band("21",21200000,9000)},
            {"text": "27", "command": lambda: set_band("27",27205000,1000)},
            {"text": "28", "command": lambda: set_band("28",28600000,1000)},
            {"text": "50", "command": lambda: set_band("50",50150000,1000)},
            {"text": "70", "command": lambda: set_band("70",74025000,12500)},
            {"text": "AIR", "command": lambda: set_band("AIR",129575000,25000)},
            {"text": "144", "command": lambda: set_band("144",145500000,25000)},
            {"text": "SAT", "command": lambda: set_band("SAT",261000000,25000)},
            {"text": "430", "command": lambda: set_band("430",433500000,25000)},
            {"text": "LPD", "command": lambda: set_band("LPD",433075000,25000)},
            {"text": "PMR", "command": lambda: set_band("PMR",446006250,25000)},
            {"text": "SHF", "command": lambda: set_band("SHF",1296200000,25000)},
        ]
...
class SMeter(tk.Frame):
    ...
    def update_squelch_threshold(self, squelch_threshold):
        ...
        # Aggiorna solo la parte relativa alla soglia
        squelch_angle = 150 - ((self.squelch_threshold) / 255) * 120
        squelch_angle = max(min(round(squelch_angle), 150), 30)

        # Ridisegna solo l'arco verde per la soglia
        self.canvas.delete("squelch_arc")  # Usa un tag per identificare l'arco
        self.canvas.create_arc(
            self.center_x - (self.radius - int(30 * self.scale_factor)),
            self.center_y - (self.radius - int(30 * self.scale_factor)),
            self.center_x + (self.radius - int(30 * self.scale_factor)),
    ...
    def update_needle(self, target_value):
        ...
        self.draw_needle(self.current_value)
        self.after(20, self.update_needle, target_value)
        
    # -----------------------------------------------------------------------------
    def calculate_angle(self, value):
        """
        Calcola l'angolo della lancetta in base al valore (0-255).
        Lo zero è a 150° (sinistra), il massimo è a 30° (destra).
        Valori oltre S9 sono compressi.
        """
        #if value <= 180:  # Valori fino a S9 (0-180 su 255)
        angle = 150 - (value / 340) * 120
        # else:  # Valori oltre S9 sono compressi (180-255 su 75)
            # angle = 30 - ((value - 180) / 1024) * 30  # Comprime i restanti 30°
        return angle
    ...
    def draw_needle(self, value):
        ...
        if self.needle_id is not None:
            self.canvas.delete(self.needle_id)  # Cancella la lancetta precedente

        # Usa calculate_angle per calcolare l'angolo
        angle = self.calculate_angle(value)  # Calcola l'angolo basato su value (0-255)
    ...
    def calculate_needle_coordinates(self, angle):
        ...
        radius = self.radius - 10  # Lunghezza della lancetta

        # Calcola le coordinate della punta della lancetta
        end_x = (center_x + radius * math.cos(math.radians(angle)))
        end_y = (center_y - radius * math.sin(math.radians(angle)))

        # Punto di inizio della lancetta (vicino al centro, per simulare il perno)
        start_x = (center_x - (radius * 0.2) * math.cos(math.radians(angle)))
        start_y = (center_y + (radius * 0.2) * math.sin(math.radians(angle)))
    ...
    def smooth_update_needle(self, target_value):
        """
        Aggiorna gradualmente la lancetta verso il valore target.
        """
        # Interrompi se il target non cambia
        if abs(self.current_value - target_value) < 0.5:
            self.current_value = target_value
            self.draw_needle(self.current_value)
            return

        # Calcolo graduale con un easing factor
        easing_factor = 0.1
        self.current_value += (target_value - self.current_value) * easing_factor

        # Disegna la lancetta e programma il prossimo aggiornamento
        self.draw_needle(self.current_value)
        self.after(20, self.smooth_update_needle, target_value)
    ...
    def calculate_smeter_value(self, livello_dB):
        ...
        smeter_value = 0  # Valore di default
    ...
    def update_smeter(self, valore):
        ...
        """
        # Filtro meno aggressivo per grandi cambiamenti
    ...
    def smooth_update_needle(self, target_value):
        """
        Aggiorna gradualmente la lancetta verso il valore target.
        Movimento rapido per grandi spostamenti e stabile per piccoli.
        """
        delta = target_value - self.current_value

        # Condizione per grandi spostamenti
        if abs(delta) > 20:  # Soglia per considerare un grande spostamento
            step = max(10, min(abs(delta) / 2, 30))  # Passo più grande
        else:
            step = max(1, abs(delta) / 5)  # Passo più piccolo per stabilità
        ...
        # Ferma l'aggiornamento se il target è stato raggiunto
        if abs(self.current_value - target_value) < 0.5:
            self.current_value = target_value
            return
